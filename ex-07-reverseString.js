
/**
 * reverseString()
 *
 * Write a function called reverseString that takes a string as input
 * and returns a string with the characters in reverse order.
 * (note: can't use .reverse() method )
 *

Escribe una función llamada reverseString que toma una cadena como entrada
 * y devuelve una cadena con los caracteres en orden inverso.
 * (nota: no se puede usar el método .reverse ())
*/

function reverseString(string1){
let nuevoString = "" 
let contador = string1.length

	for(let i = contador -1; i >= 0 ; i--){

		//console.log(string1[i])
		nuevoString += string1[i]
		
	}

	return nuevoString
}







//*-*~*~*~*~*~ Don't Touch *~*~*~*~*~*~*~*~*~*~*~
console.assert( reverseString('books') === 'skoob')
console.assert( reverseString('coolness') === 'ssenlooc')
console.assert( reverseString('bedtime') === 'emitdeb')
console.assert( reverseString('yah sure') === 'erus hay')
